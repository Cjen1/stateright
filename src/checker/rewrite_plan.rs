...
mod test {
    ...
    fn can_reindex() {
        use crate::actor::Id;
        let swap_first_and_last = RewritePlan::<Id,_>::from_values_to_sort(&vec![3, 1, 2, 0]);
        let rotate_left = RewritePlan::<Id,_>::from_values_to_sort(&vec![3, 0, 1, 2]);
        ...
        let original: VecDeque<_> = vec!['A', 'B', 'C', 'D'].into_iter().collect();
        assert_eq!(
            swap_first_and_last.reindex(&original),
            vec!['D', 'B', 'C', 'A'].into_iter().collect::<VecDeque<_>>());
        assert_eq!(
            rotate_left.reindex(&original),
            vec!['B', 'C', 'D', 'A'].into_iter().collect::<VecDeque<_>>());
    }
